...
- **Data Fetching:** Use `useFetch` / `useAsyncData` against the server proxy. Never hit Laravel API directly from client. API proxy routes live in `server/api/`.
...
- **Styling:** Design tokens defined once via Tailwind v4 `@theme` in main CSS. Implementation architecture in `.agents/rules/23_design_system.md`.
...
## Testing
- See `.agents/rules/41_workflow_testing.md` for all frontend test standards, mount patterns, and Pinia isolation.
