...
## Caching Strategy
- **Hierarchy:** OPcache (PHP) → Redis (App) → HTTP Headers → CDN → Browser.
- **Application Cache (Laravel):** Cache expensive/frequent queries, external API calls, and computed values. Do not cache simple indexed DB lookups.
  - Use `Cache::remember()`. 
  - **Null bypass hazard:** `null` returns bypass `Cache::remember()`. Wrap in a DTO if `null` is a valid value.
  - Use `Cache::tags()` (Redis) for grouping/invalidation. 
  - Invalidate via Events > Tags > Keys > TTL. Warm caches for high-traffic views.
  - Use `Cache::lock()` for expensive computations to prevent thundering herd / cache stampede.
- **TTLs:** Forever/24h+ (Static — use versioned keys `brands:v2:list`), 1-4h (Semi-static), 5-30m (Dynamic).
- **Middleware caching:** Any `share()` / `handle()` that queries the database must use `Cache::remember()`.
- **Observer registration:** Pair every cached model with `FlushCacheObserver` registration.
- **Cache key naming:** Use namespaced keys matching invalidation scope (e.g., `translations:{locale}`).
...
- **CDN page rules:** Cache static assets and SSR anonymous pages. Bypass `/api/*` and Admin routes. Purge via API (by URL/Tag). Current implementation: Cloudflare.
...
- **Assets:** Vite hashes guarantee uniqueness (`app.[hash].js`). Serve `/_nuxt/*` with `immutable` (+1 year max-age). Use `<NuxtImage>`.
