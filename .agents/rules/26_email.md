...
- **Typography:** Map fonts from `.agents/rules/11_design.md`. Use web-safe fallback stack (MJML `mj-attributes`).
- **Colors:** Pull brand palette from `.agents/rules/11_design.md`. Define once in `mj-attributes`, never inline per-notification.
...
## Logo Usage
- **Format:** PNG with transparent background. Never SVG (inconsistent rendering) or JPEG (no transparency).
- **Hosting:** Serve from public URL or CDN. Never Base64-embed (bloats payload, triggers spam filters). One logo per email (header only).
- **Sizing:** Set explicit `width` on `<mj-image>` (e.g. `width="150px"`), height auto-scales. Provide `@2x` source for retina. Always set `alt` to the project name. For dark-on-transparent logos, provide a light variant or add a white background container.

## Sending Pattern
- **Notifications only.** Use `app/Notifications/` implementing `ShouldQueue`. No raw Mailables (see `.agents/rules/21_standards_laravel.md`).
...
- **Visual QA:** Use local mail catcher (e.g., Mailpit) to preview compiled emails. Check mobile rendering manually.
...
- **Contrast:** Text/background pairings must meet WCAG AA (4.5:1 for body text, 3:1 for large text). Inherit from brand palette.
