...
## Environments (Auto-Deployed via CI/CD)
...
- Production DB backed up before running migrations (see `.agents/rules/44_workflow_database.md`).

## Post-Deploy
- Run cache commands (see `.agents/rules/25_caching_performance.md` Production Checklist).
- Restart SSR process after frontend builds.

## Rollback Strategy
- **Code:** Revert the squash-merge commit on target branch (CI/CD auto-deploys).
- **Migrations:** **Never roll back in production.** Write a new corrective migration instead.
- **Data Loss:** Restore from backup via emergency recovery procedures (`.agents/rules/44_workflow_database.md`).
