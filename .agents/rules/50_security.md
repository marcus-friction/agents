...
- **SSRF — URL Fetching:**
  - Never rely on `gethostbyname()` for URL validation (IPv4 only).
  - Use `dns_get_record($host, DNS_A | DNS_AAAA)` for address resolution.
  - Bind validated IP to cURL via `CURLOPT_RESOLVE` to prevent DNS rebinding (TOCTOU).
...
- **Rate Limiting:** Use `throttle` middleware for auth, APIs, and expensive routes. Defaults: 60/min for API, 5/min for auth endpoints.
