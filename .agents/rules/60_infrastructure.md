---
trigger: always_on
description: Environment matrix, port mappings, network binding policies, and service infrastructure.
---

# Infrastructure
> **Per-project template.** Fill in actual values during project setup.
...
- **Verify after provisioning:** Confirm all services listen on `127.0.0.1`, not `0.0.0.0` (e.g., `ss -tlnp | grep -E '6379|7700|5432'`).

## Start Scripts
- All local dev startup scripts (`serve.sh`, `start.sh`, etc.) must follow the `build-start-scripts` skill. Consult it when creating or modifying startup scripts.
