...
Best practices for Laravel, organized as an index of rule files. Each rule file
teaches what to do and why. For version-sensitive APIs, inspect the installed
framework and package versions first, then use current official Laravel
documentation through the available research tools when needed.
...
2. Map every affected concern to the rule index below. Read each mapped rule
   file before editing and skip unrelated rules. A parallel agent may explore
   independent rule groups when available; otherwise read and apply them inline
...
4. Verify version-sensitive Laravel APIs against the installed framework and
   package versions. Use current official Laravel documentation through the
   available research tools when repository evidence is insufficient.
...
Laravel authorization applies to Laravel-owned resources and operations. When a
Nuxt/Nitro server or another service owns the protected resource, keep
authorization at that resource-owning server and enforce only Laravel's own
boundary locally. Use Form Requests when input is meaningful or complex, reused,
authorization-bearing, or established by project convention; small one-off
validation may remain inline.
...
| Tests: coverage, factories, fakes, and assertions | [`rules/testing.md`](rules/testing.md) |
