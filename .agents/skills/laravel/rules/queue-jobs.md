...
For payment or other non-repeatable remote effects, persist a stable attempt
identifier and reuse it as the provider idempotency key on every retry. Protect
the corresponding local state transition with a unique constraint,
transactional claim, or equivalent atomic ownership check; unique job dispatch
alone cannot make processing idempotent.
