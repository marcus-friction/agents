...
Controllers should coordinate HTTP input, authorization, validation, an
application operation, and the response. Extract business logic when cohesion,
reuse, transactional ownership, or independent testability justifies a boundary;
do not introduce an action or service merely to satisfy an arbitrary line limit.
