...
Use policies, gates, or form request authorization for actions that depend on
the current user's permissions when Laravel owns the protected resource or
operation. Authentication alone does not establish permission, and validation
is not authorization. When another service owns and authorizes the resource,
preserve that boundary instead of inventing a duplicate Laravel policy; still
authorize any Laravel-owned action or data exposure.
