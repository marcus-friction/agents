...
if [ "${RUN_AGENT_SKILL_TESTS:-0}" != "1" ]; then
  echo "Security-review live agent cases not run"
  exit 0
fi
...
bash "$REPO_ROOT/tests/run-agent-evals.sh" \
  --case v2.security-boundaries.negative \
  --case v2.security-boundaries.positive \
  --comparison-ref HEAD \
  --runs 1 \
...
echo "Security-review live agent cases passed"
